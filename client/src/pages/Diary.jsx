...
const moodConfig = {
  // UPDATED: Softer colors for dark mode
...
const Diary = () => {
  ...
  return (
    // UPDATED: Removed hardcoded bg, added text-main
    <div className="p-8 h-screen overflow-y-auto custom-scrollbar">
      ...
      <div className="max-w-5xl mx-auto flex justify-between items-center mb-8">
        ...
        <button onClick={() => setIsModalOpen(true)} className="bg-primary hover:bg-sky-600 text-white px-6 py-3 rounded-xl font-bold flex items-center gap-2 shadow-lg shadow-sky-500/20 transition-all">
      ...
      <div className="max-w-5xl mx-auto grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {loading ? (
          <p className="text-text-muted">Loading your thoughts...</p>
        ) : entries.length === 0 ? (
          // UPDATED: Empty state styling
          <div className="col-span-full text-center py-20 bg-surface rounded-3xl border border-dashed border-border">
            <p className="text-text-muted mb-2">Your journal is empty.</p>
            <button onClick={() => setIsModalOpen(true)} className="text-primary font-bold hover:underline">Write your first note</button>
          </div>
        ) : (
          entries.map(entry => {
            ...
            return (
              ...
              <div key={entry.id} className="group bg-surface p-6 rounded-2xl border border-border shadow-card hover:border-primary/30 transition-all flex flex-col justify-between min-h-[200px]">
                <div>
                  ...
                  <p className="text-text-main text-sm leading-relaxed whitespace-pre-wrap font-sans">
                    {entry.content}
                  </p>
                </div>
          ...
          <div className="bg-surface w-full max-w-lg rounded-3xl shadow-2xl border border-border overflow-hidden animate-in zoom-in-95 duration-200">
            ...
            <div className="p-6 space-y-4">
              ...
              <textarea
                ...
                className="w-full h-40 bg-background rounded-xl p-4 text-text-main border border-border focus:outline-none focus:ring-2 focus:ring-primary/20 resize-none placeholder:text-text-muted"
                autoFocus
              />
            </div>
