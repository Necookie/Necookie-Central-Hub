...
const Diary = () => {
  ...
  const handleSave = async () => {
    ...
    const { data, error } = await supabase
      .from('journal_entries')
      .insert([{ user_id: user.id, content, mood: selectedMood }])
      .select();
  ...
  return (
    <div className="p-8 h-screen overflow-y-auto bg-slate-50">
      ...
      <div className="max-w-5xl mx-auto flex justify-between items-center mb-8">
        <div>
          <h1 className="text-3xl font-bold text-slate-800 tracking-tight">Personal Journal</h1>
          <p className="text-slate-500 mt-1">Capture your ideas, feelings, and daily reflections.</p>
        </div>
        <button 
      ...
      <div className="max-w-5xl mx-auto grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {loading ? (
          <p className="text-slate-400">Loading your thoughts...</p>
        ) : entries.length === 0 ? (
          <div className="col-span-full text-center py-20 bg-white rounded-3xl border border-dashed border-slate-300">
            <p className="text-slate-400 mb-2">Your journal is empty.</p>
            <button onClick={() => setIsModalOpen(true)} className="text-indigo-600 font-bold hover:underline">Write your first note</button>
          </div>
        ) : (
          entries.map(entry => {
              ...
              <div key={entry.id} className="group bg-white p-6 rounded-2xl border border-slate-200/60 shadow-sm hover:shadow-md transition-all flex flex-col justify-between min-h-[200px]">
                <div>
                  <div className="flex justify-between items-start mb-4">
                    <div className={`px-3 py-1 rounded-full text-xs font-bold flex items-center gap-1.5 ${style.color}`}>
                      <MoodIcon size={12} /> {style.label}
                    </div>
                    <button onClick={() => handleDelete(entry.id)} className="text-slate-300 hover:text-red-500 opacity-0 group-hover:opacity-100 transition-opacity">
                      <Trash2 size={14} />
                    </button>
                  </div>
                ...
                <div className="mt-6 pt-4 border-t border-slate-50 flex items-center gap-2 text-slate-400 text-xs font-mono">
                  <Calendar size={12} />
          ...
          <div className="bg-white w-full max-w-lg rounded-3xl shadow-2xl overflow-hidden animate-in zoom-in-95 duration-200">
            ...
            <div className="p-6 space-y-4">
                ...
                {Object.keys(moodConfig).map((moodKey) => {
                   ...
                   return (
                     <button
                       ...
                       className={`p-3 rounded-xl transition-all ${isSelected ? 'bg-indigo-600 text-white scale-110 shadow-lg' : 'bg-slate-50 text-slate-400 hover:bg-slate-100'}`}
                       title={moodKey}
                     >
                       <Icon size={20} />
                     </button>
              ...
              <textarea
                ...
                className="w-full h-40 bg-slate-50 rounded-xl p-4 text-slate-700 focus:outline-none focus:ring-2 focus:ring-indigo-500/20 resize-none"
                autoFocus
              />
            </div>
            
            <div className="p-6 bg-slate-50 border-t border-slate-100 flex justify-end gap-3">
              <button onClick={() => setIsModalOpen(false)} className="px-5 py-2.5 text-slate-500 font-bold hover:bg-slate-200 rounded-xl transition-colors">Cancel</button>
              <button onClick={handleSave} className="px-5 py-2.5 bg-indigo-600 text-white font-bold rounded-xl hover:bg-indigo-700 transition-colors shadow-lg shadow-indigo-200">Save Entry</button>
            </div>
          </div>
        </div>
      )}
