...
const WorkoutCard = () => {
  ...
  const logRun = async () => {
    ...
    try {
      ...
      // FIXED: Using 'workouts' table
      const { error } = await supabase.from('workouts').insert([{
        user_id: user.id,
        activity_type: 'Jogging',
        distance_km: parseFloat(distance),
        duration_minutes: parseInt(duration),
        calories: parseInt(duration) * 8 // Roughly 8 cals per minute
      }]);
      ...
      <div className="relative z-10">
        ...
        {mode === 'view' && (
          <div className="mt-4">
             ...
             <button onClick={() => setMode('input')} className="w-full bg-slate-900 hover:bg-emerald-600 text-white py-2 rounded-xl text-xs font-bold transition-colors flex items-center justify-center gap-2">
          ...
          <div className="mt-2 space-y-3">
            <div className="flex gap-2">
              <div className="flex-1">
                ...
                <input type="number" value={distance} onChange={(e) => setDistance(e.target.value)} className="w-full bg-slate-50 border border-slate-200 rounded-lg p-2 text-sm font-mono" placeholder="3.5" />
              </div>
              <div className="flex-1">
                <label className="text-[9px] font-bold text-slate-400 uppercase">MINS</label>
                <input type="number" value={duration} onChange={(e) => setDuration(e.target.value)} className="w-full bg-slate-50 border border-slate-200 rounded-lg p-2 text-sm font-mono" placeholder="30" />
              </div>
            </div>
            <div className="flex gap-2">
               <button onClick={() => setMode('view')} className="flex-1 py-2 text-xs text-slate-400 hover:text-slate-600">Cancel</button>
               <button onClick={logRun} disabled={loading} className="flex-[2] bg-emerald-500 hover:bg-emerald-600 text-white py-2 rounded-lg text-xs font-bold transition-colors flex items-center justify-center gap-2">
                 <Save size={14} /> Save
               </button>
            </div>
          </div>
        ...
        {mode === 'success' && (
          <div className="mt-6 flex flex-col items-center text-emerald-600 animate-pulse">
            <CheckCircle size={32} /><span className="text-xs font-bold mt-2">Run Logged!</span>
          </div>
        )}
      </div>
    </div>
