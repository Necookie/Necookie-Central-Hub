...
const DashboardHeader = () => {
  ...
  const mealMutation = useMutation({ mutationFn: (vars) => api.addMeal(vars), onSuccess: () => { queryClient.invalidateQueries(['meals']); queryClient.invalidateQueries(['history']); }});
  const workoutMutation = useMutation({ mutationFn: (vars) => api.logWorkout(vars), onSuccess: () => queryClient.invalidateQueries(['history']) });
  const taskMutation = useMutation({ 
    mutationFn: async (text) => api.addTask(text).then(async () => api.toggleTask({id: (await api.fetchTasks())[0].id, status: true})), 
  ...
  const handleInputChange = (e) => {
    ...
    else if (lower.startsWith('ran ') || lower.includes('km')) setDetectedType('workout');
  ...
  return (
    <header className="h-20 border-b border-slate-200/60 bg-white/50 backdrop-blur-md flex items-center justify-between px-6 z-40 sticky top-0 mb-6">
      <div>
        <h1 className="text-xl md:text-2xl font-light text-slate-800">Good Morning, <span className="font-bold text-sky-600">Dheyn</span></h1>
        <p className="text-xs font-mono text-slate-400 mt-1 uppercase tracking-wider">{currentDate.toLocaleDateString()} • {currentDate.toLocaleTimeString()}</p>
      </div>
      <div className="flex items-center gap-6">
        <div className="hidden md:flex items-center gap-2 bg-white border border-slate-200 px-3 py-2 rounded-xl w-96 focus-within:border-sky-500 focus-within:ring-2 focus-within:ring-sky-100 transition-all shadow-sm relative">
          ...
          <input type="text" value={status} onChange={handleInputChange} onKeyDown={handleLog} placeholder="Log anything... (e.g., 'Coding', 'Ate Pizza', 'Ran 5km')" className="bg-transparent border-none outline-none text-xs w-full text-slate-700 placeholder:text-slate-400 font-sans" />
        </div>
