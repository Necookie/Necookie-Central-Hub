        ...
        <div>
          {view === 'tracker' ? (
             ...
             <>
               ...
               <h2 className="text-3xl font-light text-slate-800 tracking-tight">{avgSleep}<span className="text-sm font-bold text-slate-400">h / night</span></h2>
             </>
          )}
        </div>
        {!isSleeping && (
        ...
        {view === 'tracker' && (
          isSleeping ? (
            <div className="mt-8">
              ...
              <button onClick={() => wakeMutation.mutate({ id: activeSleep.id, bed_time: activeSleep.bed_time })} className="w-full bg-white text-indigo-900 font-bold py-3 rounded-xl flex items-center justify-center gap-2 hover:bg-indigo-50 shadow-lg"><Sun size={18} /> I'm Awake</button>
            </div>
          ) : (
          ...
          <div className="h-40 w-full mt-2">
            <ResponsiveContainer width="100%" height="100%">
              <BarChart data={last7Days}>
                ...
                <Tooltip cursor={{fill: '#f1f5f9', radius: 4}} contentStyle={{borderRadius: '12px', border: 'none', boxShadow: '0 4px 12px rgba(0,0,0,0.1)'}} />
                <Bar dataKey="hours" radius={[4, 4, 4, 4]}>
                  {last7Days.map((entry, index) => <Cell key={index} fill={entry.hours >= 7 ? '#6366f1' : '#cbd5e1'} />)}
                </Bar>
              </BarChart>
            </ResponsiveContainer>
          </div>
